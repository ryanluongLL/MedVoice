...
function parseErrors(data) {
  if (!!data && typeof data === "object" && "errors" in data) {
    ...
    return errors.length > 0 ? errors.map(import_error3.parseError) : [];
  }
  return [];
}
function withLegacyRequestReturn(cb) {
  return async (...args) => {
    ...
    if (errors) {
      const error = new import_error3.ClerkAPIResponseError(statusText || "", {
        data: [],
        status,
        clerkTraceId,
        retryAfter
...
function handleClerkAPIError(tokenType, err, notFoundMessage) {
  if ((0, import_error4.isClerkAPIResponseError)(err)) {
    let code;
    let message;
    switch (err.status) {
      case 401:
